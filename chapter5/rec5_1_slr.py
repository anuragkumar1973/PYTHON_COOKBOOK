...
#!/usr/bin/env python3
# rec5_1_slr.py ---Recipes 81 through 88 --- Simple linear regression example: Education vs Salary
...
def test_null_hypothesis(x, y, alpha=0.05, two_sided=True):
    ...
    Returns a dict with slope, intercept, se_slope, t_stat, df, p_value (None if scipy not available), and reject (bool or None).
    """
    try:
        # Verify scipy.stats is usable; if it's not present or broken this will raise and be caught
        _ = stats.t
    except Exception:
        stats = None
    ...
    if p_value is not None:
        ...
        print("scipy not available: p-value not computed; consider installing scipy to get p-value and exact decision")
